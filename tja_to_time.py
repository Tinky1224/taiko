    ...
    with open(tja_name, 'r') as f:
        ...
        for line in f:
            if line.startswith('BPM'):
                ...
                t += abs(float(line.split(':')[-1][:-1]))
                continue
            elif line.startswith('#END'):
                break
            elif line.startswith('#MEASURE'):
                forward = int(''.join([i for i in line.split('/')[0] if i.isdigit()]))
                backward = int(''.join([i for i in line.split('/')[1] if i.isdigit()]))
                measure = forward / backward * 4
                continue
                ...
                if line[0] == ',':
                    ...
                    if ',' in line:
                        ...
                        for i in line_split:
                            ...
                            if i != '0':
                                ...
                                scroll_time.append(sec_time/1.1*4)
                    ...
                    for i in line_split:
                        if i == '0':
                            ...
                            t += note_time
            #暫時略過浮動BPM與火燒區
    return time_list, note_list, scroll_list, scroll_time